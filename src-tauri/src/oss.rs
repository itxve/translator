...
impl TargetOssPlatform for ALIYUNOSS {
    async fn oss_upload(conf: UploadInfo) -> UploadStatus {
        ...
        let target_base_path = conf.target_base_path;
        let ignore_config_js = conf.ignore_config_js;
        let files = get_all_files(dir_path, true, ignore_config_js);
        let mut upload_status = UploadStatus::new();
        for file in files.clone() {
            upload_status.increment_total();
            let file_path = file.replace(dir_path.to_str().unwrap(), "");
            let oss_path = format!("/{}{}", target_base_path, file_path);
            let rb = if let Some(mine_type) = mime_guess::from_path(&file).first() {
                RequestBuilder::new()
                    .with_expire(60)
                    .with_content_type(mine_type)
            } else {
            ...
            match oss.put_object_from_file(&oss_path, &file, rb).await {
                Ok(_) => {
                    upload_status.add_success(file.clone()); //
                }
                Err(_) => {
...
pub fn platform_list(dir: &Path) -> Vec<String> {
    list_directories(dir).unwrap()
}

#[tauri::command]
pub fn exists(dir: &Path) -> bool {
    fs::exists(dir).unwrap()
}
